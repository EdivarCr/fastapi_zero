...
async def get_users(session: Session, current_user: CurrentUser, limit=10, offset=0):
    users = await session.scalars(select(UserModel).limit(limit).offset(offset))
...
async def get_user_by_id(user_id: int, session: Session):
    user_db = await session.scalar(select(UserModel).where(UserModel.id == user_id))
